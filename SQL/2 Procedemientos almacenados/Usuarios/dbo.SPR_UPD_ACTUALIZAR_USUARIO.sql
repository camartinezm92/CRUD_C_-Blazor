...
CREATE PROCEDURE [dbo].[SPR_UPD_ACTUALIZAR_USUARIO] 
    ...
    @PI_CUSUARIO NVARCHAR(100), -- Nombre de usuario en texto plano
    @PI_CNOMBRE_COMPLETO NVARCHAR(150), -- Nombre completo del usuario
    @PI_CCONTRASENA NVARCHAR(100) = NULL, -- Nueva contraseña en texto plano (opcional)
...
BEGIN
    ...
    SET 
        USU_CUSUARIO = ENCRYPTBYKEY(KEY_GUID('ClaveEncriptacion'), @PI_CUSUARIO), 
        USU_CNOMBRE_COMPLETO = @PI_CNOMBRE_COMPLETO, 
        USU_CPASSWORD = 
            CASE 
                WHEN @PI_CCONTRASENA IS NOT NULL THEN HASHBYTES('SHA2_256', @PI_CCONTRASENA)
                ELSE USU_CPASSWORD
            END,
